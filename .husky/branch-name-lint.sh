...
if ! echo "$branch_name" | grep -Eq "$branch_name_pattern"; then
  ...
  echo ""
  echo "Branch names should follow the Conventional Commits pattern: type/description or type/number-description"
  echo "Where:"
  echo "  - type: 'feat', 'fix', 'chore', 'docs', 'style', 'refactor', 'perf', 'test', 'ci', 'build' or 'revert'"
  echo "  - number: An optional numeric identifier for the story or task"
  echo "  - description: A hyphen-separated description of the story or task"
  echo ""
  echo "Examples: feat/portfolio-update, feat/12345-add-new-feature"
  exit 1
fi
